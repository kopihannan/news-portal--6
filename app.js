...
const showNewsDisplay = (newsItem) => {
  ...
  resultItem.innerHTML = `
     <h3 class="fw-bold text-warning">Available News: ${
       totalNews ? totalNews : "Sorry! No News Data Available"
     }</h3>
  `;

  const shortedView = newsItem.sort((a, b) =>
  ...
  newsItem.forEach(function (value) {
    ...
    div.innerHTML = `
          ...
          <div class="card-body">
            ...
            <p class="card-text">${value.details
              .slice(0, 290)
              .concat("...")}</p>
            <div class="d-flex justify-content-between px-5             align-items-center">
              <div class="d-flex align-items-center">
                <img style="width:40px; height: 40px;" class="rounded-5" src="${
                  value.author.img
                }" alt="">
                <div class="ps-3"><h6>${
                  value.author.name ? value.author.name : "No Data Found"
                }</h6>
                  <h6 class="text-muted">${
          ...
          <div class="pb-3 text-center">
            <button onclick="seeMore('${
              value._id
            }')" class="btn btn-success"  data-bs-toggle="modal" data-bs-target="#see-more">see more</button>       
          </div>
